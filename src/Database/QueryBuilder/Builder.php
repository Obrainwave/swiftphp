...
class Builder
{
    ...
    private function quoteIdentifier(string|Expression $identifier): string
    {
        ...
        if (str_contains($identifier, '.')) {
            return implode('.', array_map(
                fn(string $part) => $part === '*' ? '*' : $quote . str_replace($quote, $escapeQuote, $part) . $quote,
                explode('.', $identifier)
            ));
        }

        return $quote . str_replace($quote, $escapeQuote, $identifier) . $quote;
    }
